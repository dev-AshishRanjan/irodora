...
| OQ-33 | Mockup 23 draws a seasonal label on every finished profile, and the published seasonal rule (ADR-0102) gives no summary for 3,136 of the 4,096 profiles the guided flow can produce — every one whose lightness and chroma ranges both sit in the middle; only four labels are reachable (winter deep, autumn deep, summer muted, autumn muted). What does the pill show when there is no summary — nothing, a line saying the ranges do not make one, or a later rule that names more cells? | R9 |
