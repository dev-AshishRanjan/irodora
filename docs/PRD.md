...
| OQ-36 | Mockup 15 draws four theme tiles, and the README specifies two of the themes in full — Sumi Charcoal (dark) and Washi Minimal (light). Slate Graphite and Obsidian Noir are each drawn as one swatch: `#2C323A` read from the render (L 0.315, about Sumi's level 2) and `#101114` printed (L 0.178, below Sumi's ground). F-225 must pass gates 9 and 10 over every pairing in every theme, so each needs levels, borders and text roles that no mockup draws. Derive them by a stated rule — for example Sumi's lightness steps above its ground (level 1 +0.052, level 2 +0.089, level 3 +0.133) re-anchored at each theme's ground, with the text roles held — supply the values, or ship the two as a ground change only? And is Slate Graphite's swatch its ground or its card? **MEASURED BY F-289 (2026-09-22), so this is now a choice between numbers.** The proposed derivation — Sumi’s lightness steps above its ground re-anchored at each theme’s ground, chroma and hue carried from Sumi’s corresponding step, everything else held — was run through `checkContrast` and `checkSeparation`, the real gate 9 and gate 10 code, with §4/E3’s corrected `border.strong` (#5C6472). Control: Sumi re-derives from itself exactly. RESULT — **Obsidian Noir derives cleanly: it introduces ZERO failures** (ramp #101114 / #1A1D24 / #22262E / #2B303B). **Slate Graphite introduces seven**, because its ground is lighter and contrast is spent: two are between tokens §5 specifies (foreground.2 on surface.2, 4.12 < 4.5, passes at #AEB8C4; border.strong on the ground, 2.17 < 3, passes at #737B8A) and five are on tokens §5 never lists — `ring` and `status.ok`/`status.warn`, still at their pre-R9 values, which F-225 owes a re-tune for every theme anyway. Gate 10 is 12/12 in all three. SEPARATELY, AND NOT THIS QUESTION’S: **Sumi itself — the theme that IS fully drawn — fails four pairings** (`border.strong` on levels 1–3 and on `swatch.well`, 2.0–2.64 against a required 3). That is F-225’s fifth criterion on a palette nobody has a question about, and it is worth answering before this one. Reproduce with `node mockups/tools/derive-theme.mjs`. | R9 |
