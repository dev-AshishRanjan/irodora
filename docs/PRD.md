...
| **FR-73** | **Colour combinations** — for a chosen colour, the colours that go with it, as complete wearable sets rather than a palette | Each combination names its relationship and its family, states whether it was generated or curated, and carries the gamut cost of every colour it proposes. A wearable set resolves the subject into a slot and ranks candidates for the others with the reason for each. Curated sets are classified `editorial` with full provenance and a named reviewer; **no third-party combination dataset is ingested** ([`content/AGENTS.md`](../content/AGENTS.md)). Never described as a rule about what a person should wear | R7 |
| **FR-74** | **Hold a colour against a target** — arm the Lens with a colour and read how far a real surface is from it | The comparison uses the metric set FR-48 already governs, decomposed into lightness, chroma and hue so the difference is actionable rather than one number, and it is reported with the capture conditions and confidence attached (FR-17, ADR-0005). It never asserts a match | R7 |
...
| **NFR-3** | **Determinism.** Same inputs + same versions → **the same observable value** on every platform. Every metric rounded to the precision the product displays is bit-identical; the raw doubles behind it are not, and cannot be — ECMAScript specifies `pow`, `cbrt`, `atan2`, `sin`, `cos` and `exp` as implementation-approximated, and the same Node version disagrees by 2–4 ULP on ~0.2 % of inputs between its Windows and Linux builds ([ADR-0061](adr/0061-nfr-3-guarantees-the-observable-value-not-the-double.md)) | A cross-platform test computes the same 10 000 results in Node, browser and React Native, digests them at five significant digits and asserts bitwise equality of THAT; the raw doubles are additionally held to a 16 ULP bound over 500 recorded probes, and reported without being asserted | R1 |
...
| **NFR-26** | **Every built capability is reachable.** A screen with no way in, or an engine the product never calls, is work that cannot be reviewed, cannot be used, and rots without anything reporting it | `verify-reachability` walks the route graph from the tab bar and fails on a route no interaction reaches; `verify-dead-exports` fails on a workspace package the application never imports. An exception is a recorded decision naming the feature that will consume it, and it expires with that feature — not a warning | R7 |
...
| OQ-13 | Measured type runs off the R9 scale — 01's tagline ≈ 18.5 dp, 17's slider labels ≈ 9 dp, 22's figures ≈ 46 dp — and the scale's steps were read off board 00's printed labels. Which governs: each screen's measured sizes (P3) or the declared steps? | R9 |
...
| OQ-23 | Mockups 13, 16, 17 and 18 print headings, roles and buttons inside square brackets (13's *[Capsule Solver]*, 18's seven buttons), one of them unbalanced. Are the brackets copy, or notation to drop? | R9 |
...
| OQ-29 | The serif: no mockup names it. Of the 20 serifs installed where it was measured, 01's wordmark matches Georgia Pro best (0.825) and so does its tagline (0.66) — a face under a commercial licence. F-275 then measured 20 open-licence families from google/fonts' ofl/ tree the same way, an implementing session's shortlist scored at 109 named instances (R9-MOCKUP-FIDELITY §5): **Source Serif 4 Medium** (0.747) and **Gelasio SemiBold** (0.732 — Gelasio is metric-compatible with Georgia) are closest on 01, while **Charis SIL** leads 00 (0.762) and 14 (0.793) ahead of every installed face and sits 30th on 01. No open face **among the twenty measured** reaches Georgia Pro on 01; three come within 0.08–0.10, and the tagline was not re-measured (it is three lines; the tool renders one). On 25, the only caps wordmark, the whole field is low and Georgia Pro ranks 82nd, so either that image is drawn in another face or caps at that size separate nothing. Ship Georgia Pro under its licence, or one of the open faces — and if so, which? | R9 |
...
| OQ-33 | Mockup 23 draws a seasonal label on every finished profile, and the published seasonal rule (ADR-0102, 2026.09.1) gives no summary for 3,136 of the 4,096 profiles the guided flow can produce; only four labels are reachable (winter deep, autumn deep, summer muted, autumn muted). What does the pill show when there is no summary — nothing, a line saying the ranges do not make one, or a later rule that names more cells? | R9 |
| OQ-34 | The published seasonal rule (ADR-0102, 2026.09.1) reads a range's midpoint against the phrase lexicon's edges, which classify a single colour. A guided range is spread and padded, so its midpoint never reaches the light edge (at most 0.6945 against 0.725) or the vivid edge (at most 0.092 against 0.100): the rule's *light* and *bright* cells cannot be reached from the guided flow, the rule and the profile screen's band chips name different classes for two of eight lightness and one of eight chroma answer patterns, and the only route to *deep* clears its boundary by 0.003. The rule also leaves contrast out, which conflict C12 (*from the four ranges*) did not say. By which statistic and thresholds should a range be read — the chips' bands, the lexicon's edges on another statistic, or new ones — and should contrast be read? | R9 |
| OQ-35 | Mockup 05 draws the colour-family chips Ao 青 · Aka 赤 · Midori 緑 · Murasaki 紫 · Cha 茶 · Kuro 黒 (the row runs past the screen's edge; conflict C13 names Ki 黄 as a seventh), and F-224 must put every one of the corpus's 25 families in exactly one of the seven. Charcoal sits under Kuro 黒 and the tinted greys under the hue they name, but off-white (5 entries) and the neutral greys — stone, warm-grey, cool-grey, mineral-grey (15 entries) — have no honest home: none of the seven names a white or a grey (鼠), so any placement shows a person whites or greys under a chip that says black or a hue. Add chips for 白 and 鼠 beyond the drawn edge, fold them into a stated rule, leave them out of the family filter, or something else? | R9 |
| OQ-36 | Mockup 15 draws four theme tiles, and the README specifies two of the themes in full — Sumi Charcoal (dark) and Washi Minimal (light). Slate Graphite and Obsidian Noir are each drawn as one swatch: `#2C323A` read from the render (L 0.315, about Sumi's level 2) and `#101114` printed (L 0.178, below Sumi's ground). F-225 must pass gates 9 and 10 over every pairing in every theme, so each needs levels, borders and text roles that no mockup draws. Derive them by a stated rule — for example Sumi's lightness steps above its ground (level 1 +0.052, level 2 +0.089, level 3 +0.133) re-anchored at each theme's ground, with the text roles held — supply the values, or ship the two as a ground change only? And is Slate Graphite's swatch its ground or its card? **MEASURED BY F-289 (2026-09-22), so this is a choice between numbers — with the numbers’ limits stated.** The proposed derivation (Sumi’s lightness steps above its ground re-anchored at each ground, chroma and hue carried from Sumi’s step, everything else held) was run through `checkContrast` and `checkSeparation` — **the contrast pairings and the CVD pairs, not all of gates 9 and 10**. **Obsidian Noir introduces ZERO failures** (#101114 / #1A1D24 / #22262E / #2B303B). **Slate Graphite introduces seven**: two between tokens §5 specifies, five on `ring` and `status.*`, which §5 never lists. FOUR THINGS THAT NUMBER RESTS ON: it assumes §4/E3’s conditional `#5C6472` for `border.strong` (with §5’s tabled `#464D5B` it is six, and Obsidian still zero); it is **not robust to Slate’s own ±2 ΔE00 read error** — 6 at ΔE00 1.6 below, 11 at 1.6 above, while Obsidian holds at zero across its band; 34 of the 48 pairings involve at least one token held at a pre-R9 value tuned against the old ground, which F-225 owes a re-tune anyway; and no `cvdPair` contains a token the ramp moves, so the CVD result says nothing about these themes. **The qualitative answer is robust and the counts are not: Obsidian derives cleanly, Slate spends contrast because its ground is lighter.** SEPARATELY, AND NOT THIS QUESTION’S: **Sumi itself — the theme that IS fully drawn — fails four pairings** (`border.strong` on levels 1–3 and `swatch.well`, 2.0–2.64 against 3). That is F-225’s fifth criterion on a palette nobody has a question about. Reproduce: `node mockups/tools/derive-theme.mjs`. | R9 |
| OQ-41 | Mockup 15 draws a small dot in the gutter beside each of its three engine switches (`15.engine.tabular.state` and its two siblings: 10 x 11 px, `text.tertiary`, no binding and no action) — and it draws all three switches ON. Board 00 draws a switch on and a switch off, and neither carries a dot. So the mockups show this dot in exactly one state and say nothing about the other. Two readings survive: it is decoration on every row, or it marks the on state the way 15's selected theme tile carries its own unbound dot. F-239 renders it when the switch is on, on that one piece of evidence, and records the choice here rather than presenting it as forced — the two readings fail symmetrically in the off state, and choosing between them is a preference about a drawing. Is the dot drawn on every row, or only when the switch is on? | R9 |
| OQ-40 | Mockup 13's sticky bar draws "Save Outfit to Lookbook" (`13.actions.save`, action `submit:lookbook.save`), and F-238 asks that saved outfits then be "listed where F-220's inventory places them". NO MOCKUP DRAWS A LOOKBOOK. Nothing in any inventory reads outfits back: 11's wardrobe grid lists garments, 04's "View Outfits" opens the combinations for one reading, 07 saves an outfit TO the lab, and no route lists saved outfits. This is OQ-39's question for a different object, and the two may want one answer: where does a person find what they saved — the wardrobe behind a second tab, a section on Home, an entry in the profile, or somewhere else? | R9 |
| OQ-39 | Mockup 06's app bar draws a bookmark toggle (`06.bookmark`, action `toggle:bookmark`), and F-237 asks that the saved set then be "reachable from where F-220's inventory places it". NO MOCKUP DRAWS THAT PLACE. The inventories bind a bookmark on 06 and nowhere else; 05's Atlas filters are season and family only; 23's profile draws no saved list; Home's saved-colour reader is the last Lens reading, not a set; and no route exists today. A bookmark that cannot be read back is a write with no reader, and inventing the surface is what golden rule 14 forbids. Where is the set of bookmarked colours reached from — a filter on the Atlas, a section on Home, an entry in the profile, or somewhere else? | R9 |
| OQ-38 | Mockup 18's report preview draws a signature on its envelope: the word "Signed" in a script face. The illustration set (F-229) draws pictures, and this is type — traced into paths it is a word pretending to be a drawing, and set as text it needs a script face nothing bundles (ADR-0057 bundles a Japanese subset, F-226 ships the serif). It is also the one drawing that makes a claim about a person having signed something. Trace it as art, set it in a face the product bundles, or drop the element from the report preview? | R9 |
| OQ-37 | Icons are drawn in colours that C10 does not list, and two of them cannot be drawn in one ink at all. The full list, from the icon set (F-228): 00's colour-wheel and palette buttons (a hue disc and a painter's palette, in full colour), 03's gamut chip (a hue ring), 06's editorial-review seal (green), 07, 15 and 16's padlocks (gold), 11's gap bulb (a yellow bulb, drawn like an emoji), 13's harmony score icon (three overlapping red, green and blue circles) and its capsule sparkles (gold), and 15's CVD badge (a disc split blue and amber). C10 already declares the other tinted glyphs — 02's gold HUD, 04 and 12's green verdicts, 27's tan state art. Two of these are told apart in the image ONLY by colour — the harmony circles and the CVD badge — so for them the question decides the shape as well: drawn in one ink they are three overlapping outlines and a plain disc, which is what the registry holds until it is answered. Should these carry their drawn colours as further C10 exceptions, or be drawn in ink? (OQ-15 asks the same of 00, 03, 13, 16 and 24 as a set; this is the per-icon list.) | R9 |
