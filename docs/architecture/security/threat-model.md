...
| **D** | Import bomb — a huge or deeply nested file | **NOT CONTROLLED, and this row claimed otherwise until F-286.** There is no byte limit, no record-count limit and no depth limit in `parseArchive`; the only cap in the package is `image.ts`’s per-image one, which F-286 now applies on import. Recorded as `F-288`. The row is left here saying what is true, because a control that exists only in this file is what the header of this document refuses | none yet |
