...
| **D** | Import bomb — a huge or deeply nested file | **CONTROLLED SINCE F-288**, and this row described the control for a year before it existed (`F-286`'s review found that, `F-286` marked it `none yet`). `deserialiseArchive` refuses a file past `ARCHIVE_LIMITS.maxBytes` **before `JSON.parse`** — the only place a size limit can act — refuses nesting past `maxDepth` by a bracket scan that counts outside strings, and refuses more than `maxRows` before any insert. `importArchive` carries the row bound too, because it takes `unknown` and need never have touched a file. Each limit is derived with its arithmetic where it is declared, and each is tested twice: refusing a hostile file by name, and leaving a realistic archive an order of magnitude clear — a cap that fires on real data is the failure this must not have | test |
