...
| `@irodora/color-spaces` | **also affected** — believed passing for four rounds, because its failure never made the visible ten annotations |
...
1. **The canonical digest is the gate.** Every value is rounded to **five significant
   digits** and *that* is digested and asserted **exactly**. It is what NFR-3 now promises.
   **Both** engine fixtures use it — `@irodora/color-spaces` and `@irodora/color-difference`
   — through one shared `canonicalise` in `@irodora/testing`, so the two cannot drift apart.
...
3. **The precision is a correctness property, not a preference.** Rounding does not remove a
   disagreement, it moves it: two values `d` apart differ after rounding to a grid `g` only
   if they straddle a boundary, with probability `d/g`.

   | grid | straddle per value | over ~400 000 values |
   |---|---|---|
   | 12 significant digits | 10⁻³ | certain, every run |
   | 8 | 10⁻⁷ | ~4 % |
   | **5** | **10⁻¹⁰** | **~4×10⁻⁵** |

   **Significant digits, not decimal places**, because the quantities span scales: XYZ is
   ~0.1, Lab `L` is ~50, Lab `a` and `b` reach ±100. A fixed decimal grid is far too coarse
   for one and too fine for another, and *too fine* is the direction that flakes. Five
   resolves a change of 10⁻⁵ relative — finer than anything displayed, and far finer than the
   ~10⁻⁴ a just-perceptible colour difference corresponds to.
...
- **Five significant digits is a chosen number**, justified by the straddle arithmetic rather
  than derived from a requirement. If the product ever displays more precision than that, the
  arithmetic above is what decides whether the new precision is still safe.
...
| **Round to 12 significant digits and digest that** | The instinctive choice, and wrong. The straddle probability is ~10⁻³ per value, so the digest would differ constantly and we would have replaced one flaky gate with another. This ADR's own first draft said "two decimal places", which is safe only because it happens to be coarse — the reasoning was arrived at afterwards and the number changed to follow it |
...
- **The product displays more than five significant digits** of any value. The straddle
