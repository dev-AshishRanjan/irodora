...
[`R6-EDITORIAL-DIRECTION.md`](../../docs/design/R6-EDITORIAL-DIRECTION.md) puts the Lens at **low
variance, minimal motion, medium density** — a task surface. Somebody using it is holding a phone
up to a garment and wants an answer.

### FR-18's classification had never reached a screen

`LensReading` has carried `quality: 'excellent' | 'good' | 'fair' | 'poor'` since R2.

```
grep -rn '\.quality' apps/mobile/src --include=*.tsx   →   nothing
```

What the screen showed instead was `confidence.toFixed(2)` — a bare decimal whose own type
comment reads *"Never a probability"*, and which looks exactly like one.

FR-18 says the classification is the thing that *"blocks a confident claim and returns a specific,
actionable instruction"*. **The one word a person can act on was being discarded in favour of a
number they cannot interpret.**

### One readout, where there were three elements

A "Conditions" pair, a "Confidence" pair, and — on the screen behind the sheet — an amber `Status`
carrying the instruction. Three things, one styled as a fault, describing a single capture.

Now one block: **quality · light · colour space**, then the confidence figure subordinate with its
scale stated, then the instruction under **To improve it**.

The order is FR-17's and has not moved — conditions before value, *"because a number shown first
has already been read by the time its qualifier arrives"*. **A test asserts the order, not just
the presence**: a presence-only test passes on exactly the arrangement the requirement forbids.

The number stays. It is the honest ceiling FR-15 produces, and removing it would hide the one
figure that says how far the engine is willing to go.

### The reticle had the swatch keyline problem, and nobody had seen it

It was a single 2px rule in `border.strong` — a token for drawing edges on **our own surfaces**,
where the other side of the line is a ground we chose. Over a live camera image the other side is
an arbitrary colour, which is precisely the case F-068 proved a single tone cannot handle: a
single hairline measured **1.00 against its own colour**, *"not a weak edge but NO EDGE AT ALL"*.
On the one surface where the marker must always be findable, it was nearly invisible over a pale
garment.

And it was **closed on all four sides**. A hard rule around a colour changes how that colour
reads — simultaneous contrast is the entire reason `swatch.well` exists, and this was that hazard
applied to the live subject somebody is judging.

It is now **two-tone corner marks**, reusing the gamut-verified pair rather than inventing one:
the better tone reaches 4.23 against the worst possible sample and the two differ by ~18:1
whatever sits behind them. A new pair would have carried no such evidence, and **nothing here
could have measured it — the camera image is not a token** (E-072).

Its position is derived from `REGION_FRACTION`. The old overlay hard-coded `left: '45%'` and
`width: '10%'` beside a fraction of `0.1` — two statements of one fact, and the marks would have
gone on pointing at the old area the moment the sampled region moved. **A reticle that lies about
where the colour is read is worse than none: it is an instruction to aim somewhere the engine is
not looking.**

### Removing one component emptied a whole subsystem

`Status kind="warn"` on the Lens was the **only** `Status` in the product. Its exemption read
*"two of three"* and named **F-149 itself** as the feature that would close it.

So the entry predicted this feature would finish the job, and this feature undid it. Then the
first replacement owner was wrong too, and the same check caught it: the entry said outfit scoring
(F-031) would be the first surface to show a status, and **F-031 is done without having shown
one** — the exact shape ADR-0088 describes. The owner is now F-152, whose criterion 2 is *"every
screen has a designed empty, loading and error state"*; an error state **is** a `bad` status.

The distinction the whole entry turns on, now written down (E-071):

> A **status** says something is GOOD or BAD. A **readout** says what the conditions WERE.

The Lens describes. It does not judge.

### What this feature deliberately did not do

**The sheet is F-158's and it stays.** Criterion 3 was discharged there; re-opening it would be
doing the same work twice.

**The viewfinder does not go full-bleed.** F-148's `padding` prop makes it possible, and E-065
records that a second call site is the signal the pattern needs *a rule rather than a prop* —
which is a different piece of work. A task surface does not need the bleed; the colour page did.
...
|---|---|
| `pnpm verify:ci` — all 32 runnable CI steps | **PASS** |
| 677 mobile · 123 ui | **PASS** |
| claims lint · token reach and its proof | **PASS** |

Font subset regenerated for the new Japanese copy — the content gate caught it, as it did
yesterday.

### Still owed

**Nobody has pointed this at a garment.** Both attested criteria are outstanding for that reason,
and the specific risk is one no test here can create: whether corner marks at a tenth of the frame
are visible enough **in bright daylight**, which is the condition this surface is most used in.

**The readout could read as a table rather than a readout.** Five stacked label/value rows are
structurally correct and might be tedious. The failure mode is not loudness but dullness, and if
it reads as a table the answer is fewer lines — not a return to three elements.

---

## 2026-09-04 — FIX · eight red checks, one cause, and the command that would have shown them

Reported: [gate 16 on the APK lane](https://github.com/dev-AshishRanjan/irodora/actions/runs/33876401246)
and [CI](https://github.com/dev-AshishRanjan/irodora/actions/runs/33876130894), both on `551c1b1`.

CI failed at **step 7 of 33** — `verify-token-reach --prove` — so the twenty-six steps after it
had not run for several pushes. Fixing the reported failure uncovered the next, seven times.

### Every failure, and when it actually broke

| # | check | rotted when | mine? |
|---|---|---|---|
| 1 | `verify-token-reach --prove` | F-143/F-145 gave `surface.1` literal readers | yes |
| 2 | `eslint scripts` — `png.mjs`, `verify-peer-deps.mjs` | F-142, F-143 | yes |
| 3 | `verify-app-imports` | F-145 added tests that quote an import | yes |
| 4 | `verify-empty-assertions` | F-146 wrote `toEqual([])` | yes |
| 5 | `generate-e2e-flows --check` | F-146 removed `home.openAtlas` | yes |
| 6 | `verify-worklet-reach --prove` | F-138 changed a callee's signature | yes |
| 7 | `verify-spacing-scale --prove` | F-146 rewrote the file it planted into | yes |
| 8 | `test:content` — 7 codepoints | Japanese copy outgrew the font subset | yes |

**All eight are mine, and none is subtle.** Each says exactly what is wrong the first time it is
run. Nothing ran them.

### The cause, which is not any of the eight

`gates.json` has 15 gates. CI runs those **plus twenty-odd mutation proofs and generator
`--check` steps that no gate names** — correctly, because a proof belongs to the check it proves
rather than to a gate. The gap was that **there was no way to ask what CI would say without
pushing.**

`pnpm verify:ci` closes it. The commands are read out of `.github/workflows/ci.yml` — derived,
never copied, because a hand-maintained list here would drift from the workflow, which is the
same disease one level along.

This had happened before. The **2026-09-03** entry above — *"CI had been red for four pushes,
and every local run was green"* — ends by running every workflow step by hand, once, and
recording that they passed. That fixed the instance and left the class, and four features later
the rot was back.

### Three proofs had rotted the same way, and two had been "fixed" by re-rotting

Each planted a mutation into source, anchored on a literal transcribed from that source.

- **Spacing** planted into `<View style={{ gap: 4` until F-140 tokenised the screens. The fix
  re-anchored on the one raw `style={{}}` Home still carried. F-146 removed that one too. **Both
  fixes re-anchored on a construct the design system is actively deleting** — each bought one
  release. It now **appends** a synthetic declaration to a screen chosen at runtime: it needs the
  file to be scanned and needs nothing of its contents.
- **Worklets** held a callee's whole signature as a string. F-138 removed a parameter default —
  because a worklet cannot read a captured variable from one — and the marker stopped matching.
  It now names a **function**, which survives every change to parameters and return type. It is
  anchored to the start of a line, because F-138's own docblock quotes the emitted form of that
  very function and an unanchored search matched the comment.
- **Token reach** asserted that removing `nativeElevation` leaves `surface.1` unreached — true
  only while nothing named it as a literal, which the comment said out loud. F-143 and F-145 both
  added one. Re-picking another token would rot identically, so it is now **two halves**: literal
  readers removed with the map kept must stay quiet, and with the map also removed must fire.
  **The first half is the assertion the old case never made** — that resolving *through* the map
  counts as reaching.

### A false positive, and it mattered more than the outage

`verify-app-imports` flagged three imports in `screens.test.tsx` that were not imports. They were
strings inside `expect(route).toContain("from '../../../src/store/repository'")` — assertions
*about* another file's import, resolved from the test's own directory, where nothing is.

This gate exists because Metro resolves differently from `tsc`. It earns that by being believed,
and three findings that cannot be fixed because nothing is wrong is how a gate gets a
`--no-verify` habit built around it. It now ignores a specifier that sits inside a string
literal, with decoys both ways — including **a real broken import on a line that also carries a
string**, so the new rule cannot become a hiding place.

Also found: a **raw NUL byte** in `screens.test.tsx`, where the source meant ` `. Legal
JavaScript, which is why nothing failed — and enough to make `grep` answer *"Binary file
matches"* and nothing else.

### The journey walked to a button that no longer exists

`atlas.journey.json` tapped `home.openAtlas`, one of the ten buttons Home carried until F-146.
F-145 had already made the Atlas a **tab**. The generator caught it — the key was not in the
catalogue — which is the point of generating flows from keys rather than strings.

Retargeting it surfaced a real ambiguity the generator refused: the tab reads **Atlas** and the
screen is titled **Colour Atlas**, so a text selector matches two elements and *a tap must not
choose*. The alternatives were renaming product copy to suit a test, or tapping something
ambiguous. Instead the tabs carry `tab-<route>` ids and the generator learned a `testID`
selector — **checked against the source that declares it**, so an id that no longer exists fails
here rather than on a phone.

---

## Gate 16 — the icon was there all along

```
✗ no launcher icon in the artefact
  nothing under res/ matches an icon or splash resource. Either the config lost its
  `icon` key, or prebuild did not run
```

**Both named causes were wrong.** `app.config.ts` has `icon`, `adaptiveIcon` and
`expo-splash-screen`; the assets are committed; prebuild runs with `--clean`; and the generated
project contains `mipmap-hdpi/ic_launcher.webp` and nine `splashscreen_logo.png` files.

`ICON_ENTRY` matches `res/mipmap-hdpi-v4/ic_launcher.webp` — which is what an APK contains until
it is a **release** build. AGP runs `optimizeReleaseResources` by default and it **shortens
resource paths**: every file under `res/` is renamed to something like `res/xa.webp`. This lane
switched from `assembleDebug` to `assembleRelease` — for good reasons, recorded in the workflow —
and the check was written against the artefact it used to produce.

The name scan stays as the fast path; when it finds nothing, `aapt2 dump badging` is asked what
the launcher would load. Those are the paths Android itself resolves through `resources.arsc` —
not a second opinion, but the only source of truth once a name has been discarded. **It does not
quietly pass when aapt2 is absent**: it says the check could not be performed and why.

**Also fixed, before it bites:** both lanes ran `$ANDROID_HOME/build-tools/36.0.0/apksigner`
while gate 16 reported cross-checking against **37.0.0** on the same job. Which version is
installed is the runner image's choice. Both now take the newest and fail loudly if there is
none.
...
| ran | result |
|---|---|
| `pnpm verify:ci` — all 32 runnable CI steps, in order | **PASS** |
| gate mirror · gate 0 | **PASS** |

**Not run: gate 16 itself.** `verify-apk.mjs --prove` builds its fixtures with `aapt2` and
`android.jar` and refuses without them — *"a proof that cannot build its fixtures is not a proof
that passed"* — and this machine has no Android SDK. What was verified locally is that the file
parses, lints and formats, and that the two `badging` patterns extract five icon paths from a
realistic release excerpt, deduped, without picking up `launchable-activity`'s empty `icon=''`.

**The fix itself is unverified until the lane runs again.** The diagnosis rests on: the resources
are provably in the generated project, `shrinkResources` is off, and the lane changed variant —
which leaves path shortening as the only mechanism that fits. That is a strong argument and it is
not a measurement.

### Still owed

**The mark is not actually checked on a release artefact.** `carriesMark` decodes PNG, and a
release APK re-encodes icons to WebP, so `readable` is empty and the gate emits a note instead of
an assertion. It has always been honest about this. It means the hard assertion is *"an icon
resource exists"*, not *"it is ours"* — and closing that needs either a WebP decoder or reading
`resources.arsc` directly.

**`verify-ci` is not a CI replica.** It runs the same commands in the same order on this machine.
The last incident was gate 0 resolving a link into a `node_modules` the runner did not have, and
this would not have caught it.
