...
|---|---|---|---|
| 2026-08-20 | **`scripts/*.mjs` is not linted at all.** `pnpm lint` is `turbo run lint`, which invokes each PACKAGE's `eslint .` — and `scripts/` is in no package, so nothing reaches it. Confirmed directly: `npx eslint scripts/verify-state.mjs` fails to parse, because `project: true` finds no tsconfig covering it. That is ~20 files including every gate script (`verify-state`, `verify-contrast`, `verify-guards`, `verify-engine-purity`, `verify-claims`), i.e. the code that decides whether everything else is allowed to ship. Found while wiring `packages/ui`, whose own `jest.config.mjs` hit the same parse error — that one is now linted without type-awareness, which is the shape the fix should take here too. | gate blind spot | open |
