/**
 * GENERATED — do not edit.
 *   source: docs/design/design-system.manifest.json
 *   regenerate: pnpm --filter @irodora/design-tokens generate
 *
 * React Native accepts #RRGGBB and rgba() only. A translucent token carries both its
 * rgba() form and `composited` — the pre-composited hex over its declared base, blended
 * in LINEAR LIGHT, which is the value the contrast gate checked.
 */

export const nativeColors = {
  dark: {
    background: '#090807',
    'surface.1': '#12100F',
    'surface.2': '#1A1817',
    'surface.3': '#22211F',
    'swatch.well': '#2B2A28',
    'swatch.hairline': '#F6F5F3',
    foreground: '#F6F4F1',
    'foreground.2': '#A5A39F',
    'foreground.3': '#82807C',
    link: '#F6F4F1',
    border: 'rgba(255, 255, 255, 0.08)', 'border.on.background': '#515151', 'border.on.surface.1': '#525252', 'border.on.surface.2': '#545454', 'border.on.surface.3': '#575656',
    backdrop: 'rgba(0, 0, 0, 0.6)', 'backdrop.on.background': '#040303', 'backdrop.on.surface.1': '#080706', 'backdrop.on.surface.2': '#0D0C0B', 'backdrop.on.surface.3': '#131211',
    'border.strong': '#7B7977',
    inverse: '#F6F4F1',
    'inverse.foreground': '#0C0B09',
    ring: '#719DC4',
    'status.ok': '#49AB79',
    'status.warn': '#D58D25',
    'status.bad': '#FEAAAC',
    'chart.1': '#F5F5F5',
    'chart.2': '#B7B7B7',
    'chart.3': '#808080',
    'chart.4': '#525252',
    'chart.5': '#303030',
    'swatch.hairline.inverse': '#131110',
  },
  light: {
    background: '#FDFCF9',
    'surface.1': '#FFFFFF',
    'surface.2': '#F7F6F3',
    'surface.3': '#F0EEEB',
    'swatch.well': '#ECEAE7',
    'swatch.hairline': '#131110',
    foreground: '#171411',
    'foreground.2': '#5E5A56',
    'foreground.3': '#8D8A87',
    link: '#171411',
    border: 'rgba(0, 0, 0, 0.08)', 'border.on.background': '#F4F3F0', 'border.on.surface.1': '#F6F6F6', 'border.on.surface.2': '#EEEDEA', 'border.on.surface.3': '#E7E6E3',
    backdrop: 'rgba(0, 0, 0, 0.4)', 'backdrop.on.background': '#C9C9C7', 'backdrop.on.surface.1': '#CBCBCB', 'backdrop.on.surface.2': '#C5C4C2', 'backdrop.on.surface.3': '#BFBEBC',
    'border.strong': '#817F7D',
    inverse: '#171411',
    'inverse.foreground': '#FDFCF9',
    ring: '#426D95',
    'status.ok': '#307450',
    'status.warn': '#905B06',
    'status.bad': '#7C000C',
    'chart.1': '#161616',
    'chart.2': '#484848',
    'chart.3': '#7A7A7A',
    'chart.4': '#ABABAB',
    'chart.5': '#D4D4D4',
    'swatch.hairline.inverse': '#F6F5F3',
  },
  'fuka.dark': {
    background: '#06090B',
    'surface.1': '#0D1114',
    'surface.2': '#16191C',
    'surface.3': '#1E2225',
    'swatch.well': '#2B2A28',
    'swatch.hairline': '#F6F5F3',
    foreground: '#EFF6FB',
    'foreground.2': '#9EA4A9',
    'foreground.3': '#7B8186',
    link: '#EFF6FB',
    border: 'rgba(255, 255, 255, 0.08)', 'border.on.background': '#515151', 'border.on.surface.1': '#525253', 'border.on.surface.2': '#535455', 'border.on.surface.3': '#555758',
    backdrop: 'rgba(0, 0, 0, 0.6)', 'backdrop.on.background': '#020304', 'backdrop.on.surface.1': '#050709', 'backdrop.on.surface.2': '#0A0D0F', 'backdrop.on.surface.3': '#101315',
    'border.strong': '#767A7E',
    inverse: '#EFF6FB',
    'inverse.foreground': '#080C0E',
    ring: '#719DC4',
    'status.ok': '#49AB79',
    'status.warn': '#D58D25',
    'status.bad': '#FEAAAC',
    'chart.1': '#F5F5F5',
    'chart.2': '#B7B7B7',
    'chart.3': '#808080',
    'chart.4': '#525252',
    'chart.5': '#303030',
    'swatch.hairline.inverse': '#131110',
  },
  'fuka.light': {
    background: '#F9FCFF',
    'surface.1': '#FFFFFF',
    'surface.2': '#F1F7FB',
    'surface.3': '#EAEFF3',
    'swatch.well': '#ECEAE7',
    'swatch.hairline': '#131110',
    foreground: '#111619',
    'foreground.2': '#565C60',
    'foreground.3': '#858C90',
    link: '#111619',
    border: 'rgba(0, 0, 0, 0.08)', 'border.on.background': '#F0F3F6', 'border.on.surface.1': '#F6F6F6', 'border.on.surface.2': '#E8EEF2', 'border.on.surface.3': '#E1E7EB',
    backdrop: 'rgba(0, 0, 0, 0.4)', 'backdrop.on.background': '#C6C9CB', 'backdrop.on.surface.1': '#CBCBCB', 'backdrop.on.surface.2': '#C0C5C8', 'backdrop.on.surface.3': '#BABFC2',
    'border.strong': '#7B8084',
    inverse: '#111619',
    'inverse.foreground': '#F9FCFF',
    ring: '#426D95',
    'status.ok': '#307450',
    'status.warn': '#905B06',
    'status.bad': '#7C000C',
    'chart.1': '#161616',
    'chart.2': '#484848',
    'chart.3': '#7A7A7A',
    'chart.4': '#ABABAB',
    'chart.5': '#D4D4D4',
    'swatch.hairline.inverse': '#F6F5F3',
  },
  'yama.dark': {
    background: '#0B0706',
    'surface.1': '#140F0E',
    'surface.2': '#1C1816',
    'surface.3': '#25201E',
    'swatch.well': '#2B2A28',
    'swatch.hairline': '#F6F5F3',
    foreground: '#FBF2EF',
    'foreground.2': '#A9A19E',
    'foreground.3': '#867E7C',
    link: '#FBF2EF',
    border: 'rgba(255, 255, 255, 0.08)', 'border.on.background': '#515151', 'border.on.surface.1': '#535252', 'border.on.surface.2': '#555453', 'border.on.surface.3': '#585656',
    backdrop: 'rgba(0, 0, 0, 0.6)', 'backdrop.on.background': '#040302', 'backdrop.on.surface.1': '#090606', 'backdrop.on.surface.2': '#0F0C0B', 'backdrop.on.surface.3': '#151110',
    'border.strong': '#7E7876',
    inverse: '#FBF2EF',
    'inverse.foreground': '#0E0A09',
    ring: '#719DC4',
    'status.ok': '#49AB79',
    'status.warn': '#D58D25',
    'status.bad': '#FEAAAC',
    'chart.1': '#F5F5F5',
    'chart.2': '#B7B7B7',
    'chart.3': '#808080',
    'chart.4': '#525252',
    'chart.5': '#303030',
    'swatch.hairline.inverse': '#131110',
  },
  'yama.light': {
    background: '#FFFBF9',
    'surface.1': '#FFFFFF',
    'surface.2': '#FBF4F2',
    'surface.3': '#F4EDEA',
    'swatch.well': '#ECEAE7',
    'swatch.hairline': '#131110',
    foreground: '#191312',
    'foreground.2': '#605957',
    'foreground.3': '#908986',
    link: '#191312',
    border: 'rgba(0, 0, 0, 0.08)', 'border.on.background': '#F6F2F0', 'border.on.surface.1': '#F6F6F6', 'border.on.surface.2': '#F2EBE9', 'border.on.surface.3': '#EBE4E2',
    backdrop: 'rgba(0, 0, 0, 0.4)', 'backdrop.on.background': '#CBC8C7', 'backdrop.on.surface.1': '#CBCBCB', 'backdrop.on.surface.2': '#C8C3C1', 'backdrop.on.surface.3': '#C2BDBB',
    'border.strong': '#847E7C',
    inverse: '#191312',
    'inverse.foreground': '#FFFBF9',
    ring: '#426D95',
    'status.ok': '#307450',
    'status.warn': '#905B06',
    'status.bad': '#7C000C',
    'chart.1': '#161616',
    'chart.2': '#484848',
    'chart.3': '#7A7A7A',
    'chart.4': '#ABABAB',
    'chart.5': '#D4D4D4',
    'swatch.hairline.inverse': '#F6F5F3',
  },
  'aota.dark': {
    background: '#070906',
    'surface.1': '#0F110E',
    'surface.2': '#171A16',
    'surface.3': '#20221E',
    'swatch.well': '#2B2A28',
    'swatch.hairline': '#F6F5F3',
    foreground: '#F2F6EF',
    'foreground.2': '#A1A49E',
    'foreground.3': '#7E817B',
    link: '#F2F6EF',
    border: 'rgba(255, 255, 255, 0.08)', 'border.on.background': '#515151', 'border.on.surface.1': '#525252', 'border.on.surface.2': '#545453', 'border.on.surface.3': '#565755',
    backdrop: 'rgba(0, 0, 0, 0.6)', 'backdrop.on.background': '#030402', 'backdrop.on.surface.1': '#060806', 'backdrop.on.surface.2': '#0C0D0A', 'backdrop.on.surface.3': '#111310',
    'border.strong': '#787B76',
    inverse: '#F2F6EF',
    'inverse.foreground': '#0A0C08',
    ring: '#719DC4',
    'status.ok': '#49AB79',
    'status.warn': '#D58D25',
    'status.bad': '#FEAAAC',
    'chart.1': '#F5F5F5',
    'chart.2': '#B7B7B7',
    'chart.3': '#808080',
    'chart.4': '#525252',
    'chart.5': '#303030',
    'swatch.hairline.inverse': '#131110',
  },
  'aota.light': {
    background: '#FAFDF8',
    'surface.1': '#FFFFFF',
    'surface.2': '#F4F7F1',
    'surface.3': '#EDF0EA',
    'swatch.well': '#ECEAE7',
    'swatch.hairline': '#131110',
    foreground: '#131611',
    'foreground.2': '#595C56',
    'foreground.3': '#898C86',
    link: '#131611',
    border: 'rgba(0, 0, 0, 0.08)', 'border.on.background': '#F1F3EF', 'border.on.surface.1': '#F6F6F6', 'border.on.surface.2': '#EBEEE9', 'border.on.surface.3': '#E4E7E2',
    backdrop: 'rgba(0, 0, 0, 0.4)', 'backdrop.on.background': '#C8C9C6', 'backdrop.on.surface.1': '#CBCBCB', 'backdrop.on.surface.2': '#C2C5C0', 'backdrop.on.surface.3': '#BDBFBA',
    'border.strong': '#7E817C',
    inverse: '#131611',
    'inverse.foreground': '#FAFDF8',
    ring: '#426D95',
    'status.ok': '#307450',
    'status.warn': '#905B06',
    'status.bad': '#7C000C',
    'chart.1': '#161616',
    'chart.2': '#484848',
    'chart.3': '#7A7A7A',
    'chart.4': '#ABABAB',
    'chart.5': '#D4D4D4',
    'swatch.hairline.inverse': '#F6F5F3',
  },
} as const;

export const nativeRadius = {
  xs: 6,
  sm: 10,
  md: 14,
  lg: 20,
  xl: 28,
  pill: 999,
  swatchRatio: 0.25,
} as const;

/**
 * The spacing scale, by name. `nativeSpacing.md` is CSS's `--space-md`.
 *
 * NAMED RATHER THAN POSITIONAL, and F-103 is why. This was an array, so a component
 * wrote `nativeSpacing[2]` and nothing in that expression named 12. ADR-0074 removed
 * a 14 and added 12 and 16, moving every index above 1 — safe only because nothing
 * read the scale yet. Five components read it by the time this was fixed, and a
 * shifted index would have compiled, passed every test, and handed a style prop a
 * perfectly valid wrong number.
 *
 * Removing or renaming a step now fails `typecheck` at every call site that wanted
 * it. Every step is a multiple of `spacing.base`, and
 * scripts/verify-spacing-scale.mjs fails if that stops being true.
 */
export const nativeSpacing = {
  xs: 4,
  sm: 8,
  md: 12,
  lg: 16,
  xl: 20,
  xl2: 28,
  xl3: 40,
  xl4: 56,
  xl5: 96,
} as const;
export const nativeTapTarget = 44 as const;
/**
 * The smallest a sample may be drawn where the screen asks you to JUDGE a colour, in dp.
 *
 * Derived from the CIE 2° standard observer at the declared viewing distance — the same
 * observer this product's colorimetry uses throughout. A list that RANKS may keep its
 * thumbnails; a pair that has to be assessed reaches this (ADR-0095).
 */
export const nativeJudgeableSample = 77 as const;

/** Absolute points, NOT the manifest ratios — RN lineHeight is a length. */
export const nativeType = {
  latin: {
    'display.1': { fontSize: 72, lineHeight: 70.56, letterSpacing: -2.88, fontWeight: '500' },
    'display.2': { fontSize: 34, lineHeight: 35.7, letterSpacing: -1.02, fontWeight: '500' },
    title: { fontSize: 22, lineHeight: 26.4, letterSpacing: -0.44, fontWeight: '600' },
    body: { fontSize: 15, lineHeight: 24.75, letterSpacing: 0, fontWeight: '400' },
    small: { fontSize: 13, lineHeight: 20.15, letterSpacing: 0, fontWeight: '400' },
    xs: { fontSize: 11.5, lineHeight: 17.25, letterSpacing: 0, fontWeight: '400' },
    label: { fontSize: 10, lineHeight: 14, letterSpacing: 1.6, fontWeight: '600', textTransform: 'uppercase' },
  },
  japanese: {
    'display.1': { fontSize: 72, lineHeight: 79.11, letterSpacing: -2.88, fontWeight: '500' },
    'display.2': { fontSize: 34, lineHeight: 40.03, letterSpacing: -1.02, fontWeight: '500' },
    title: { fontSize: 22, lineHeight: 29.6, letterSpacing: -0.44, fontWeight: '600' },
    body: { fontSize: 15, lineHeight: 27.75, letterSpacing: 0, fontWeight: '400' },
    small: { fontSize: 13, lineHeight: 22.59, letterSpacing: 0, fontWeight: '400' },
    xs: { fontSize: 11.5, lineHeight: 19.34, letterSpacing: 0, fontWeight: '400' },
    label: { fontSize: 10, lineHeight: 15.7, letterSpacing: 1.6, fontWeight: '600', textTransform: 'uppercase' },
  },
} as const;

/** Scale steps at or above the 18.66px WCAG large-text floor. */
export const nativeLargeTextSizes = ['display.1', 'display.2', 'title'] as const;
/** Scale steps BELOW it. A largeText-only token may never be used at these. */
export const nativeSmallTextSizes = ['body', 'small', 'xs', 'label'] as const;
export const nativeLargeTextMinPx = 18.66 as const;

/** iOS Dynamic Type curve per step, matched by SIZE to Apple's ramp. */
export const nativeDynamicTypeRamp = {
  'display.1': 'largeTitle',
  'display.2': 'largeTitle',
  title: 'title2',
  body: 'subheadline',
  small: 'footnote',
  xs: 'caption1',
  label: 'caption2',
} as const;

/** ONE family per script — RN has no fallback cascade. jp is bundled; Latin is the platform. */
export const nativeFamilies = { jp: 'NotoSansJP' } as const;

export const nativeNumericFeature = 'tabular-nums' as const;

/** Tonal. Each level names the surface token it resolves to; there is no shadow. */
export const nativeElevation = {
  '0': 'background',
  '1': 'surface.1',
  '2': 'surface.2',
  '3': 'surface.3',
} as const;

export const nativeMotion = {
  durations: {
    micro: 120,
    local: 180,
    view: 260,
  },
  easing: {
    out: [0.16, 1, 0.3, 1],
    inOut: [0.65, 0, 0.35, 1],
  },
  animatable: ['opacity', 'transform'],
  forbidden: ['background-color on a swatch', 'cross-fade between samples', 'width', 'height', 'top', 'left'],
} as const;

/** Used when the platform expresses no preference — NOT a hard-coded light. */
export const nativeDefaultTheme = 'dark' as const;
