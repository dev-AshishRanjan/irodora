# Render a word in each installed serif and score it against a crop of a mockup by mask IoU.
param([string]$img, [int]$x, [int]$y, [int]$w, [int]$h, [string]$word, [string]$polarity = 'light')
...
$rows = @()
foreach ($f in $fams) {
  try { $font = New-Object System.Drawing.Font($f, 72, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel) } catch { continue }
  if ($font.Name -ne $f) { continue }
  ...
  $rows += [pscustomobject]@{ face = $f; iou = [Math]::Round($inter / [Math]::Max(1, $union), 3); aspect = [Math]::Round($ra, 2) }
  $bmp.Dispose()
}
"target aspect $([Math]::Round($aspect,2))  grid ${N}x${M}"
$rows | Sort-Object iou -Descending | Select-Object -First 8 | ForEach-Object { "{0,-22} IoU {1}  aspect {2}" -f $_.face, $_.iou, $_.aspect }
